...
export default function FooterInfo() {
  ...
  const { address, isMainnet } = React.useMemo(() => {
    ...
    const addr = preferMain ? (envMain || typhoonMain?.typhoon) : (envSep || typhoonTestnet?.typhoon)
    return { address: addr, isMainnet: preferMain }
  }, [chain?.id])

  if (!mounted || !address) return null
  const base = isMainnet ? 'https://starkscan.co' : 'https://sepolia.starkscan.co'
  ...
  return (
    <div className="mt-0 text-xs text-muted-foreground flex items-center justify-center gap-4 w-full text-center">
      <div className="truncate">
        Contract:
        <a href={href} target="_blank" rel="noreferrer" className="ml-2 underline text-accent font-mono break-all">
          {address}
        </a>
      </div>
      <a
